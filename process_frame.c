...
void ChangeDetection() {
	...
	for(r = Border*nc; r < (nr-Border)*nc; r += nc) {
		...
		for(c = Border; c < (nc-Border); c++) {
			...
			int32 dx = - (int32) *(p-nc-1) + (int32) *(p-nc+1)-2* (int32) *(p-1) + 2* (int32) *(p+1)- (int32) *(p+nc-1) + (int32) *(p+nc+1);
			/* implement Sobel filter in y-direction */
			int32 dy = - (int32) *(p-nc-1) - 2* (int32) *(p-nc)- (int32) *(p-nc+1) + (int32) *(p+nc-1)+2* (int32) *(p+nc) + (int32) *(p+nc+1);
...
void DrawBoundingBoxes() {
	for(uint16 i = 0; i < ImgRegions.noOfObjects; i++) {
		if(ImgRegions.objects[i].area > MinArea) {
			DrawBoundingBox(ImgRegions.objects[i].bboxLeft, ImgRegions.objects[i].bboxTop,
					ImgRegions.objects[i].bboxRight, ImgRegions.objects[i].bboxBottom, false, GREEN);
			//get pointer to root run of current object
			struct OSC_VIS_REGIONS_RUN* currentRun = ImgRegions.objects[i].root;
			uint8 ind;
			...
			DrawString(ImgRegions.objects[i].centroidX-10, ImgRegions.objects[i].centroidY, 7, LARGE, RED, out_str);
		}
	}
}
